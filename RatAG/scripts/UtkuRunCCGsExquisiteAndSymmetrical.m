function UtkuRunCCGsExquisiteAndSymmetrical(dayNo,nullFlag)
...
%         spikeTimeIndxCell2 = round((res2-onsetTime/1e6)*fs) + 1;
        ...
        pairStr = ['AG' dayNo ' - ' num2str(cellID1) cell1type ' (ch: ' num2str(ch1) ', sh: ' num2str(shank1) ', prb:' num2str(probe1) ', clu: ' num2str(clu1) ', reg: ' region1 ')' ' v ' ...
...
%             legend(cellstr(num2str(cell2num(probegroup.channel_id(find((probegroup.shank_id + 1) == shank2)))' + 1, 'ch%-d')),'Location','West')
    ...
    if nullFlag
        if day == 1
            save([dataPath 'AG_2019-12-23_NSD' '/' 'nullGroupStatsRatAGday' dayNo '.mat'],'pairGroupStatTable')
        elseif day == 2
            save([dataPath 'AG_2019-12-27_NSD' '/' 'nullGroupStatsRatAGday' dayNo '.mat'],'pairGroupStatTable')
        end 
    else
        if day == 1
            save([dataPath 'AG_2019-12-23_NSD' '/' 'groupStatsRatAGday' dayNo '.mat'],'pairGroupStatTable')
        elseif day == 2
            save([dataPath 'AG_2019-12-27_NSD' '/' 'groupStatsRatAGday' dayNo '.mat'],'pairGroupStatTable')
        end 
    end
