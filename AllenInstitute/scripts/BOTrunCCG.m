     ...
     for loopAnimal = 1:length(animalsList)
        ...
        for loopProbe = 1:length(probeList)
            ...
            for loopBrainRegions = 1:length(brainRegionList)
                ...
                if ~isempty(dataJscale1.pairs.ExcPairs) && ~isempty(dataJscale5.pairs.ExcPairs)
                    ExcPairs = [dataJscale1.pairs.ExcPairs(:,1:2); dataJscale5.pairs.ExcPairs(:,1:2)];
                else
                    ExcPairs = [];
                end 

                if ~isempty(dataJscale1.pairs.InhPairs) && ~isempty(dataJscale5.pairs.InhPairs)
                    InhPairs = [dataJscale1.pairs.InhPairs(:,1:2); dataJscale5.pairs.InhPairs(:,1:2)];
                else
                    InhPairs = [];
                end

                if ~isempty(dataJscale1.pairs.GapPairs) && ~isempty(dataJscale5.pairs.GapPairs)
                ...
                for loopPairs = 1:size(allPairs,1)
                    ...
                    if plotFlag
                        ...
                        pairStr = ['Mouse: ' num2str(animalID) ' - ' num2str(cellID1) cell1type cell1opto ' (ch: ' num2str(ch1) ', probe: ' probe1 ', region: ' region1 ')' ' v ' ...
        ...
        if nullFlag
            save([dataPath 'groupStatsMouse' num2str(animalID) 'NULL.mat'],'pairGroupStatsTable')
        else
            save([dataPath 'groupStatsMouse' num2str(animalID) '.mat'],    'pairGroupStatsTable')
        end
