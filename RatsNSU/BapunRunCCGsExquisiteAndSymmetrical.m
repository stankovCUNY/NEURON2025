function BapunRunCCGsExquisiteAndSymmetrical(RatID,nullFlag)
    ...
    for loopPairs = 1:size(filteredPairs,1)
        ...
        pairStr = ['Rat' RatID ' (exq and sym) - ' num2str(cellID1) cell1type ' (ch: ' num2str(ch1) ', sh: ' num2str(shank1) ')' ' v ' ...
...
%             legend(cellstr(num2str(cell2num(probegroup.channel_id(find((probegroup.shank_id + 1) == shank2)))' + 1, 'ch%-d')),'Location','West')
    ...
    if nullFlag
        save([dataPath 'Rat' RatID '/' 'nullGroupStatsRat' RatID '.mat'],'pairGroupStatTable')
    else
        save([dataPath 'Rat' RatID '/' 'groupStatsRat'     RatID '.mat'],'pairGroupStatTable')
    end
