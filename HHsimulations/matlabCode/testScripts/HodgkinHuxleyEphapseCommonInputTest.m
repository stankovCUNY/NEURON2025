function HodgkinHuxleyEphapseCommonInputTest(timeMin)
...
%     close all
    ...
    noiseSc = 0.015; % normal noise scale
...
%     I_current_tar(1,50*(1/fs)) = 0; % reset first fifty ms to 0
    ...
    for i = 1:length(t)
        ...
        % Calculating the input
        Input_ref = I_current_ref(i) - (I_Na_ref(i) + I_K_ref(i) + I_Leak_ref(i)) + juncSwitch(i)*g_GJref*(Vtar(i)-Vref(i));
        Input_tar = I_current_tar(i) - (ionSwitch(i)*(I_Na_tar(i) + I_K_tar(i)) + I_Leak_tar(i)) + juncSwitch(i)*g_GJtar*(Vref(i)-Vtar(i));
...
%     title('CI v T and CI v R')
    ...
    nexttile
    plot(tR*1000,ccgRT(:,1,2),'k','LineWidth',2)
    xlim([-20 20])
    xlabel('[ms]')
    ylabel('Counts')
%     title('R v T')
...
%     hold off
    ...
    [~,evokedSpikelet1] = waveformAvg(Vref',stimPeak1idx,preLength,postLength,fpass,fs,filterFlag,noDemeanFlag);            
    [~,evokedSpikelet2] = waveformAvg(Vref',stimPeak2idx,preLength,postLength,fpass,fs,filterFlag,noDemeanFlag);  
