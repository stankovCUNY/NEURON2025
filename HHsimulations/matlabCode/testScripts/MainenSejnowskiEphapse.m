function MainenSejnowskiEphapse(timeMin,inputType)
...
%     close all
    ...
    noiseSc = 0.05; % normal noise scale

    Vrest = -70; % mV − change this to −65 if desired
    dt = 1/30; % ms
    fs  = (1/dt)*1000; % hz
...
%     g_Leak = 0.3; % mS/cm^2
    ...
    for i = 1:length(t)
        ...
        conductance_K_ref(i)   = g_K*(nRef(i));
...
%     waveIdx = r + LL;
    ...
    for i = 1:length(t)
        ...
        % Calculating the input
        Input = juncSwitch(i)*I_current_tar(i) - (ionSwitch(i)*(I_Na(i) + I_K(i)) + I_Leak(i));
...
function [alpha_m, beta_m] = m_equations(V, Vrest)
    alpha_m = (0.02*(V-20))/(1-exp(-(V-20)/9));
    beta_m = (-0.002*(V-20))/(1-exp((V-20)/9));
end

% calculate alpha n and beta n based on Table 2
function [alpha_n, beta_n] = n_equations(V, Vrest)
    alpha_n = (0.182*(V+35))/(1-exp(-(V+35)/9));
    beta_n = (-0.124*(V+35))/(1-exp( (V+35)/9));
end

% calculate alpha h and beta h based on Table 2
function [alpha_h, beta_h] = h_equations(V, Vrest)
    alpha_h = (0.024*(V+50))/(1-exp(-(V+50)/5));
    beta_h = (-0.0091*(V+75))/(1-exp((V+75)/5));
end