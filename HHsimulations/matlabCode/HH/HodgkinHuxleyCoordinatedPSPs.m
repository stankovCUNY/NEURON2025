function HodgkinHuxleyCoordinatedPSPs(timeSec)
    
    tic

%     close all
    
    time = timeSec*60; % in seconds
    ...
    delayPSP = 0.003*fs; % units are in sec
    ...
    for i = 1:length(t)
        ...
        Vref(i+1)  = Vref(i)*(1 + noiseSc*normrnd(0,1))  + Input_Ref*  dt*(1/C); 
    ...
    sTarTemp1 = zeros(1,length(t));
    sTarTemp2 = zeros(1,length(t));
    
    sTarTemp1(spikeTimesRef) = 1;
    sTarTemp2(spikeTimesRef + delayPSP) = 1;
    
    sTarTemp1 = conv(sTarTemp1,sWave);
    sTarTemp2 = conv(sTarTemp2,-sWave);
    
    sTarTemp1 = sTarTemp1(1:length(t));
    sTarTemp2 = sTarTemp2(1:length(t));
    
    sTar = sTarTemp1 + sTarTemp2;
...
%     tiledlayout(3,1)
    ...
    xlabel('[ms]')
    ylabel('Vm [mV]')
    title('Ref v T and Ref v R')
    set(gca,'FontSize',6)
    
    nexttile(7)
    plot((-(preLength-1):postLength)*dt*1000,PSP,'LineWidth',4) 
%     xlim([0 2000])
    xlabel('[\musec]')
%     ylabel('Vm [mV]')
	title('pulse')
...
%     set(gca, 'YDir','reverse')
    ...
    nexttile(12)
    plot((-(preLength-1):(postLength-1))*dt*1000,diff(PSP),'LineWidth',4)
%     xlim([0 2000])
    xlabel('lag times [\musec]')
%     ylabel('counts')
    title('d(pulse)/dt')
    set(gca,'FontSize',6)
