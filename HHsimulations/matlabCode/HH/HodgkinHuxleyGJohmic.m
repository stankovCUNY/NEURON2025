function HodgkinHuxleyGJohmic(timeMin,DC)
...
%     I_current = 10*sin(2*pi*F*t/1000);
    ...
    for i = 1:length(t)
        ...
        Input_ref = I_current_ref(i) - (I_Na_ref(i) + I_K_ref(i) + I_Leak_ref(i)) + ~juncOff*juncSwitch(i)*g_GJtar*(Vtar(i)-Vref(i));
%         Input_tar = I_current(i) - (I_Na_tar(i) + I_K_tar(i) + I_Leak_tar(i)) + gC*(Vref(i)-Vtar(i));
        Input_tar = I_current_tar(i) - (I_Na_tar(i) + I_K_tar(i) + I_Leak_tar(i)) + ~juncOff*juncSwitch(i)*g_GJtar*(Vref(i)-Vtar(i));
        
        % Calculating the new membrane potential
%         Vref(i+1) = Vref(i)*(1 + frozenNoiseRef(i)) + Input_ref* dt*(1/C);
%         Vtar(i+1) = Vtar(i)*(1 + noiseSc*normrnd(0,1)) + Input_tar* dt*(1/C);
...
%         end
    ...
    preLength         = 5*30;
    postLength        = 5*30;
    refAP             = waveformAvg(Vref',       spikeTimesRef(1)*fs,preLength,postLength,fpass,fs,filterFlag,noDemeanFlag);
    refDiffAP         = waveformAvg(diff(Vref'), spikeTimesRef(spikeTimesRef < 60)*fs,preLength,postLength,fpass,fs,filterFlag,noDemeanFlag);        
    couplingPotential = waveformAvg(Vtar',spikeTimesRef(spikeTimesRef < 60)*fs,preLength,postLength,fpass,fs,filterFlag,noDemeanFlag);        
    stimWaveInput     = waveformAvg(Input_tar',  spikeTimesRef(spikeTimesRef < 60)*fs,preLength,postLength,fpass,fs,filterFlag,noDemeanFlag); 
    stimWaveCurrent   = waveformAvg(g_GJtar*(Vref'-Vtar'),  spikeTimesRef(spikeTimesRef < 60)*fs,preLength,postLength,fpass,fs,filterFlag,noDemeanFlag); 
    
%     nexttile(1)
%     plot((-(preLength-1):postLength)*dt,refAP*1000,'k','LineWidth',1)
%     xlim([-3 3])
%     xlabel('[ms]')
%     ylabel('[mV]')
%     title('Ref. intrawave')
% %     plot((-(preLength-1):postLength)*dt,stimWaveCurrent*1000,'k','LineWidth',1)
% %     xlim([-2.5 2.5])
%     set(gca,'FontSize',5)
