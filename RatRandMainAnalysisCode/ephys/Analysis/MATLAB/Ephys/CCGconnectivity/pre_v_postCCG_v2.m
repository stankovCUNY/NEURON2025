function [pairs_plot_all, pp_pairs] = pre_v_postCCG_v2(spike_data_fullpath, ...
...
% 
%   script by Nat Kinsky, modified by Atanas Stankov and Orestis Stefanis 
...
ip.addParameter('nullFlag', false, @islogical);
...
if ~debug
if nullFlag
    load(spike_data_fullpath, 'spikesNULL')
    spikes = spikesNULL;
else
        ...
        if ~exist('pairs','var')  % the logic in the if/else statement is terrible.
            if nullFlag
                load(fullfile(data_dir,[session_name '_jscale' num2str(jscale_use(js)) '_alpha' ...
                    num2str(round(alpha*100)) '_pairs' save_name_append '_NULL']), ...
                    'pairs', 'jscale', 'alpha')
            else
                load(fullfile(data_dir,[session_name '_jscale' num2str(jscale_use(js)) '_alpha' ...
...
if strcmp(screen_type, 'two_prong')
    ...
    loopCellPairType = {'ExcPairs', 'InhPairs', 'GapPairs'};
    % concatenate pairs 
    for k = 1:nepochs
        for l = 1:length(loopCellPairType)
            pairs(k).(loopCellPairType{l}) = cat(1, pairs_comb(1).pairs(k).(loopCellPairType{l}),...
                                          pairs_comb(2).pairs(k).(loopCellPairType{l}));

            % Identify unique pairs
            pair_row_inds = arrayfun(@(a,b) find(all(pairs(k).(loopCellPairType{l})(:,1:2) == [a b],2)), ...
                pairs(k).(loopCellPairType{l})(:,1), pairs(k).(loopCellPairType{l})(:,2), ...
            ...
            pairs(k).(loopCellPairType{l}) = pairs(k).(loopCellPairType{l})(unique_pairs,:);
            
            %log both jscale values
            pairs(k).jscale = [1 5];
        end
    end
    
    tempFilename = [session_name '_' screen_type '_jscale' num2str(jscale_use(1)) 'and' ... 
                    num2str(jscale_use(2)) '_alpha' num2str(round(alpha*100)) '_pairs' save_name_append];
    if ~isfile([tempFilename '.mat'])
        save(fullfile(data_dir,tempFilename), ...
                'pairs', 'jscale_use', 'alpha')
    end
...
if plot_jitter || plot_conv
    ...
    elseif screensize(3) >= 2560 && screensize(4) >= 1440 
        res_type = 'QHD';
        pos = [70 230 2660 1860]; a_offset = [0 850 100 900]'; b_offset = [0 0 -100 -100]';
    else
        res_type = 'HD';
%         pos = [35 115 1160 630]; a_offset = [0 50 700 800]'; b_offset = [0 -50 0 -50]';
        pos = [35 115 1920 1080]; 
    end
...
jitter_data = struct(); % initialize anyhow
...
%                 duration = 0.02; binSize = 1/SampleRate; jscale_plot = 5;
            elseif coarse_fine == 2 % fine
                duration = 0.007; binSize = 1/SampleRate; jscale_plot = 1;
            end
            
%             if nfig > 1  % set up new figures
                try close fig_use.Number; end; try close fig_use.Number; end
...
%                 make_figure_pretty(fig_use.Number);
%                 arrayfun(@(a) make_figure_pretty(a), ...
%                     hcomb)
...
%% loop via column of figure creation
function loop_k(k,...
                nplot, ...
                npairs_all, ...
                hw2, ...
    ...
    cell1_shank = bz_spikes.shankID(bz_spikes.UID == cell1);
    cell2_shank = bz_spikes.shankID(bz_spikes.UID == cell2);
    ...
    elseif plot_jitter
        [GSPExc,GSPInh,pvalE,pvalI,ccgR,tR,LSPExc,LSPInh,JBSIE,JBSII] = ...
            CCG_jitter(res1, res2, SampleRate, binSize, duration, 'jscale', jscale,  'plot_flag', true, ...
            'plot_output', get(fig_use, 'Number'), 'subfig', epoch_plot + (k-1)*nepochs_plot, ...
            'subplot_size', [nrows, 3], 'njitter', njitter, 'alpha', alpha,...
            'for_grant', for_grant, 'use_subplot', use_subplot);
        ...
        if save_jitter
           ...
           save_jitter_vars(jitter_data, [cell1, cell2], {res1,res2}, ...
                           rows_to_plot(k), epoch_plot,data_dir, ... 
                           epoch_names, session_name, conn_type, jscale, alpha, ...
                           GSPExc,GSPInh,pvalE,pvalI,ccgR,tR,LSPExc,LSPInh,JBSIE,JBSII, ...
                           cell1_type,cell2_type,cell1_shank,cell2_shank);
