%% SCRIPT RUNS BUT NEEDS SPECIFIC LINES OTHERWISE IT CRASHES. CHEERS, -A
...
for j = 1:size(pairsAna,1)
    ...
    subplot(1,2,1)
    edges = 0:1/fs:0.1;
    h = histogram(diff(A),                  edges,'Normalization','probability');
    
    y = fft(h.Values);
    n = length(h.Values);          % number of samples
    f = (0:n-1)*(fs/n);     % frequency range
    power = abs(y).^2/n;    % power of the DFT

    plot(f,power)
    
    [pxx,f,pxxc] = periodogram(h.Values,[],length(h.Values),fs,'ConfidenceLevel',0.95);
    ...
    periodogram(h.Values,rectwin(length(h.Values)),length(h.Values),fs)
    
    hold on
%     histogram(diff(setdiff(A*1000,AvBcondA*1000)),edges,'Normalization','probability')
%     histogram(diff(setdiff(A*1000,AvBcondA*1000)),edges,'Normalization','probability')
    hold off
    xlim([0 15])
%     
%     subplot(1,2,2)
%     edges = 0:1/30:50;
%     histogram(diff(B*1000),                  edges,'Normalization','probability')
%     hold on
%     histogram(diff(setdiff(B*1000,AvBcondB*1000)),edges,'Normalization','probability')
%     hold off
%     xlim([0 15])
    ...
    [~,~,~,~,ccgRsync,~,~,~,~,~] = ...
      CCG_jitter(setdiff(A,AvBcondA),setdiff(B,AvBcondB),fs,binSize,duration,'jscale',jscale, 'plot_flag', true, ...
                'plot_output', get(fig_use, 'Number'), 'subfig', 1, ...
                'subplot_size', [3, 1], 'njitter', njitter, 'alpha', alpha,...
                'for_grant', for_grant);        
    ...
    line(tRms,ccgRsync(:,1,1),'color','r','LineWidth',1)
    ylabel('Count')
