...
for j = 1:size(cell_pairs_mat_unique,1)
    ...
    ylims = get(gca,'ylim');
    if any(GSPExc); hold on; plot(tR(GSPExc == 1)*1000, 1.01*ylims(2), 'r^'); end
    if any(GSPInh); hold on; plot(tR(GSPInh == 1)*1000, 1.01*ylims(2), 'bv'); end
    set(gca,'FontSize',9)
    ...
    nexttile
    [GSPExc,GSPInh,~,~,~,tR,~,~,~,~] = ...
          CCG_jitter(C_group,AvBcondA,fs,binSize,duration,'jscale',jscale,'plot_flag', true, ...
                    'plot_output', get(fig_use, 'Number'), ...
                    'njitter', njitter, 'alpha', alpha,...
                    'for_grant', for_grant);
    ...
    nexttile
    [GSPExc,GSPInh,~,~,~,tR,~,~,~,~] = ...
          CCG_jitter(C_group,AvBcondB,fs,binSize,duration,'jscale',jscale, 'plot_flag', true, ...
                    'plot_output', get(fig_use, 'Number'), ...
                    'njitter', njitter, 'alpha', alpha,...
                    'for_grant', for_grant);
    ...
    nexttile
    [GSPExc,GSPInh,~,~,~,tR,~,~,~,~] = ...
          CCG_jitter(C_group,A,fs,binSize,duration,'jscale',jscale, 'plot_flag', true, ...
                    'plot_output', get(fig_use, 'Number'), ...
                    'njitter', njitter, 'alpha', alpha,...
                    'for_grant', for_grant);
    ...
    nexttile
    [GSPExc,GSPInh,~,~,~,tR,~,~,~,~] = ...
          CCG_jitter(C_group,B,fs,binSize,duration,'jscale',jscale, 'plot_flag', true, ...
                    'plot_output', get(fig_use, 'Number'), ...
                    'njitter', njitter, 'alpha', alpha,...
                    'for_grant', for_grant);
