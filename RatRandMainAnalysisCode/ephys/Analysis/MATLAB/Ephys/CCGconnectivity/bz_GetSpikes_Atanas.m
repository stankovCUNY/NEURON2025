function spikes = bz_GetSpikes_Atanas(varargin)
...
for i=1:length(cluFiles) 
    ...
    for c = 1:length(cells)
       ...
       if any(getWaveforms)
           ...
           spikes.allSpikeRawWaveforms{count} = squeeze(wav(ind,bb,:));
           spikes.allSpikeRawWaveformsAllChans{count} = squeeze(wav(ind,:,:));
