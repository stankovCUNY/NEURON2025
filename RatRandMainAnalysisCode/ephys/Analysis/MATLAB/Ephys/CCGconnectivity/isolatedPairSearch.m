...
%% get channels from waveform files. Not computaitonally savvy but it's fastest solution
...
for j = 1:size(pairsAna,1)
    ...
    refShNoiseST      = load([datapath 'RoyMaze_shank' num2str(n1shank) '_spikeTimesNoise']);
    refShValidUnitsST = load([datapath 'RoyMaze_shank' num2str(n1shank) '_spikeTimesValidUnits']);
    refShValidUnitsST = setdiff(refShValidUnitsST.validUnits,res1);
    refShankAllST     = [refShNoiseST.noiseCluster; refShValidUnitsST];
    
    tarShNoiseST      = load([datapath 'RoyMaze_shank' num2str(n2shank) '_spikeTimesNoise']);
    tarShValidUnitsST = load([datapath 'RoyMaze_shank' num2str(n2shank) '_spikeTimesValidUnits']);
    tarShValidUnitsST = setdiff(tarShValidUnitsST.validUnits,res2);
    ...
    [GSPExc,GSPInh,pvalE,pvalI,ccgR,tR,LSPExc,LSPInh,JBSIE,JBSII] = ...
      CCG_jitter(res1,tarShankAllST,fs,binSize,duration,'jscale',jscale, ...
                    'plot_flag', true, ...
                    'plot_output', get(fig_use, 'Number'), ...
                    'njitter', njitter, 'alpha', alpha,...
                    'for_grant', for_grant,  'plot_pointwiseBands',false);
    ...
    [GSPExc,GSPInh,pvalE,pvalI,ccgR,tR,LSPExc,LSPInh,JBSIE,JBSII] = ...
      CCG_jitter(refShankAllST,res2,fs,binSize,duration,'jscale',jscale, ...
                    'plot_flag', true, ...
                    'plot_output', get(fig_use, 'Number'), ...
                    'njitter', njitter, 'alpha', alpha,...
                    'for_grant', for_grant,  'plot_pointwiseBands',false);
