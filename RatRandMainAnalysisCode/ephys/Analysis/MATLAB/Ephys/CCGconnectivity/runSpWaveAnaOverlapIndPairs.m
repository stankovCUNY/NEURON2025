...
for h = 1:size(pairsAna,1)
    
    pairAna = pairsAna(h,:);
    binGroup1 = binGroup1All{h};
    binGroup2 = binGroup2All{h};
    ...
    tiledlayout(8,2, 'Padding', 'none', 'TileSpacing', 'compact');
...
%         subplot(8,2,plotIdx(i))
    ...
    %     xlabel('[ms]')
        ...
        ylim([-3 2])
        title(sprintf('%d%s channel %d (Max chan: %d)', ...
                    jit_var_out(pair_idx(2)).cell_pair(1), ...
                    jit_var_out(pair_idx(2)).cell1type, ...
                    (RoySession1.shankID(waveIdxCell1)-1)*8+i, ...
                    RoySession1.maxWaveformCh(waveIdxCell1)+1))
...
%         ylabel('[mV]')
        ...
        title(sprintf('%d%s channel %d (Max chan: %d)', ...
                    ...
                    jit_var_out(pair_idx(2)).cell2type, ...
                    (RoySession1.shankID(waveIdxCell2)-1)*8+i, ...
                    RoySession1.maxWaveformCh(waveIdxCell2)+1))
    ...
    save_file = fullfile(data_dir, [sprintf('waveformAna_%dv%d_allChans_Ver3',pairAna(1),pairAna(2)) session_name '_jscale' num2str(jscale) '.pdf']);
