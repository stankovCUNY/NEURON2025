...
for j = 1:size(pairsAna,1)
    ...
    [T3pre,T2pre,T1pre,T1post,T2post,T3post] = surroundingSpikes(res1,res2);
    ...
    [~,~,~,~,ccgR,tR,~,~,~,~]         = CCG_jitter(res1,res2,fs,binSize,duration,'jscale',jscale, 'plot_flag', false, ...
                                            ...
                                            'for_grant', for_grant);
    [~,~,~,~,ccgR_T3pre,~,~,~,~,~]    = CCG_jitter(res1,T3pre,fs,binSize,duration,'jscale',jscale, 'plot_flag', false, ...
                                            'plot_output', get(fig_use, 'Number'), 'subfig', 1, ...
                                            'subplot_size', [3, 1], 'njitter', njitter, 'alpha', alpha,...
                                            'for_grant', for_grant);
    [~,~,~,~,ccgR_T2pre,~,~,~,~,~]    = CCG_jitter(res1,T2pre,fs,binSize,duration,'jscale',jscale, 'plot_flag', false, ...
                                            'plot_output', get(fig_use, 'Number'), 'subfig', 1, ...
                                            'subplot_size', [3, 1], 'njitter', njitter, 'alpha', alpha,...
                                            'for_grant', for_grant);
    [~,~,~,~,ccgR_T1pre,~,~,~,~,~]    = CCG_jitter(res1,T1pre,fs,binSize,duration,'jscale',jscale, 'plot_flag', false, ...
                                            'plot_output', get(fig_use, 'Number'), 'subfig', 1, ...
                                            'subplot_size', [3, 1], 'njitter', njitter, 'alpha', alpha,...
                                            'for_grant', for_grant);
    [~,~,~,~,ccgR_T1post,~,~,~,~,~]   = CCG_jitter(res1,T1post,fs,binSize,duration,'jscale',jscale, 'plot_flag', false, ...
                                            'plot_output', get(fig_use, 'Number'), 'subfig', 1, ...
                                            'subplot_size', [3, 1], 'njitter', njitter, 'alpha', alpha,...
                                            'for_grant', for_grant);
    [~,~,~,~,ccgR_T2post,~,~,~,~,~]   = CCG_jitter(res1,T2post,fs,binSize,duration,'jscale',jscale, 'plot_flag', false, ...
                                            'plot_output', get(fig_use, 'Number'), 'subfig', 1, ...
                                            'subplot_size', [3, 1], 'njitter', njitter, 'alpha', alpha,...
                                            'for_grant', for_grant);
    [~,~,~,~,ccgR_T3post,~,~,~,~,~]   = CCG_jitter(res1,T3post,fs,binSize,duration,'jscale',jscale, 'plot_flag', false, ...
                                            'plot_output', get(fig_use, 'Number'), 'subfig', 1, ...
                                            'subplot_size', [3, 1], 'njitter', njitter, 'alpha', alpha,...
                                            'for_grant', for_grant);
    ...
    line(tRms,ccgR(:,1,2),        'color','k','LineWidth',4)
    hold on
    patchline(tRms,ccgR_T3pre(:,1,2),  'EdgeColor','r','LineWidth',2,'EdgeAlpha',0.1,'FaceColor','r','FaceAlpha',0.1)
    patchline(tRms,ccgR_T2pre(:,1,2),  'EdgeColor','r','LineWidth',2,'EdgeAlpha',0.5,'FaceColor','r','FaceAlpha',0.5)
    line(tRms,ccgR_T1pre(:,1,2),       'color',    'r','LineWidth',2)
    line(tRms,ccgR_T1post(:,1,2),      'color',    'b','LineWidth',2)
    patchline(tRms,ccgR_T2post(:,1,2), 'EdgeColor','b','LineWidth',2,'EdgeAlpha',0.5,'FaceColor','b','FaceAlpha',0.5)
    patchline(tRms,ccgR_T3post(:,1,2), 'EdgeColor','b','LineWidth',2,'EdgeAlpha',0.1,'FaceColor','b','FaceAlpha',0.1)
    hold off
    ...
    legend({'All','T3pre','T2pre','T1pre','T1post','T2post','T3post'},'Location','NorthEast')

    save_file = fullfile(datapath, ['Exq_pairs_CCG_surroundingSpk' session_name]);
