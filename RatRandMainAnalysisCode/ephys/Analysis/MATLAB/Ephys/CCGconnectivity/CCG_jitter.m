...
%  Modified by A Stankov (2021-2025).

function [GSPExc,GSPInh,pvalE,pvalI,ccgR,tR,LSPExc,LSPInh,JBSIE,JBSII,ccgjm,ccgj]=...
    CCG_jitter(res1,res2,SampleRate,BinSize,Duration,varargin)
    ...
    ip.addParameter('microsecFlag', false, @islogical);
    ip.addParameter('plot_ACbands', true, @islogical);
    ip.addParameter('axisLabels',   true, @islogical);
    ip.addParameter('plot_pointwiseBands', true, @islogical)
    ip.addParameter('use_subplot', false, @islogical);
    ip.addParameter('norm_flag', false, @islogical);
    ip.addParameter('lineWidth', 2, @(a) isnumeric(a) && a > 0); 
    ip.addParameter('colorPlot', 'k');
    ip.addParameter('stylePlot', '-', @ischar);
    ...
    microsecFlag        = ip.Results.microsecFlag;
    plot_flag           = ip.Results.plot_flag;
    plot_ACbands        = ip.Results.plot_ACbands;
    plot_pointwiseBands = ip.Results.plot_pointwiseBands;
    use_subplot         = ip.Results.use_subplot;
    norm_flag           = ip.Results.norm_flag;
    lineWidth           = ip.Results.lineWidth;
    axisLabels          = ip.Results.axisLabels;
    colorPlot           = ip.Results.colorPlot;
    stylePlot           = ip.Results.stylePlot;
    ...
    %     end
        ...
        if isempty(nn)
            ...
            delta = one_ms*jscale; % jitter interval
            parfor ii=1:njitter
...
%                 res2_jitter = round((res2 + 2*(one_ms*jscale)*rand(size(res2))-1*one_ms*jscale)*SampleRate)/SampleRate;
                res2_jitter = floor(res2/delta)*delta + rand(size(res2))*delta;
...
%         line(tRms,a_sim,'linestyle','--','color','r')
%         line(tRms,b_sim,'linestyle','--','color','r')
...
%         end
        if plot_ACbands
            if norm_flag
                patch([tRms; flipud(tRms)],[aSim/length(res1);   flipud(bSim/length(res1))],   [200 200 200]/255,'EdgeAlpha',0); % simultaneous bands

                if plot_pointwiseBands
                    patch([tRms; flipud(tRms)],[aPoint/length(res1); flipud(bPoint/length(res1))], [125 125 125]/255,'EdgeAlpha',0); % point-wise bands
                end
                
                line(tRms,ccgjm/length(res1),      'color','w','LineWidth',lineWidth)
                    
            else
                patch([tRms; flipud(tRms)],[aSim;   flipud(bSim)],   [200 200 200]/255,'EdgeAlpha',0); % simultaneous bands

                if plot_pointwiseBands
                    patch([tRms; flipud(tRms)],[aPoint; flipud(bPoint)], [125 125 125]/255,'EdgeAlpha',0); % point-wise bands
                end

                line(tRms,ccgjm,      'color','w','LineWidth',lineWidth)
            end
        end 
        
        if norm_flag
            if strcmp(colorPlot,'k') && strcmp(stylePlot,'-')
                line(tRms,ccgR(:,1,2)/length(res1),'color','k',      'LineWidth',lineWidth)
            else
                line(tRms,ccgR(:,1,2)/length(res1),'color',colorPlot,'LineWidth',lineWidth,'lineStyle',stylePlot)
            end
        else
            if strcmp(colorPlot,'k') && strcmp(stylePlot,'-')
                line(tRms,ccgR(:,1,2),'color','k',      'LineWidth',lineWidth)
            else
                line(tRms,ccgR(:,1,2),'color',colorPlot,'LineWidth',lineWidth,'lineStyle',stylePlot)
            end
        end
        
%         bar(tRms,ccgR(:,1,2),'EdgeColor','k','FaceAlpha',0)
