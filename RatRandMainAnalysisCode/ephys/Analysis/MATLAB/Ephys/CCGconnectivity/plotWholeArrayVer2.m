...
for j = 2:size(pairsAna,1)
    ...
    refUnitSpikeLat   = res1;
    tarUnitStackSynch = oneRefUnitOneTargetShankCCGs(refUnitSpikeLat,n1shank);
    
    for i = 1:size(tarUnitStackSynch,2)
        if ~isempty(tarUnitStackSynch{i})
            tarResNoise = tarUnitStackSynch{i};
            Lratio(i) = LratioHiro(refUnitSpikeLat,tarResNoise,n1shank);
        end
    end
