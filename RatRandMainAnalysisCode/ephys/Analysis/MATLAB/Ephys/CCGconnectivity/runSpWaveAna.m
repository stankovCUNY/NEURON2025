...
for j = 1:size(pairsAna,1)
    ...
    for k = 1:length(binGroup1{idx})
        line([tR(binGroup1{idx}(k))*1000,tR(binGroup1{idx}(k))*1000],ylims,'Color',[0 0.4470 0.7410],'LineWidth',2)
    end
    for k = 1:length(binGroup2{idx})
        line([tR(binGroup2{idx}(k))*1000,tR(binGroup2{idx}(k))*1000],ylims,'Color',[0.8500 0.3250 0.0980],'LineWidth',2)
    end
    hold off
