function [CCG,spikeTimesInBin] = CCGtrackedSpikeTimes(R,T,binSize,lagLimit)
...
%     filtUnion   = diffUnion(diffUnion < halfWin);
    ...
    for i = 1:length(allIdxFilt)
        ...
        for j = 1:length(delta)
            ...
            % quick hacky fix to a error outside of CCG lag time bounds
            if isempty(find(intLagTimes == CCGlag(j)))
                continue
            end
